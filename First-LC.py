...
def main():
    ...
    print(demosimple.__doc__) # Print the docstring of the demosimple function
    demosimple() 
...
def demosimple():
    ...
    model = ChatGoogleGenerativeAI(model="gemini-2.5-pro",  google_api_key=os.getenv("GOOGLE_API_KEY"), temperature=0.5)

    # Create the Chain
    chain = prompt | model     # LCEL - LangChain Expression Language
    
    # Invoke (run) the Chain - The Chat Model returns a Message
    print(chain.invoke({"name": "Abraham Lincoln"}).content) #content : this function returns the content of the message
